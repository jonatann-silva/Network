...
import (
	...
	handler "jonatann-silva/network/agent/adapter/http"
	middleware "jonatann-silva/network/agent/adapter/http/middleware"
	conn "jonatann-silva/network/agent/conn"
	client "jonatann-silva/network/client"
	dsn "jonatann-silva/network/dsn"
	config "jonatann-silva/network/dsn/structs/config"
	http "jonatann-silva/network/pkg/http"
...
// New creates a new DSN agent from the configuration,
...
func (a *Agent) setupServer() error {
	...
	server, err := dsn.NewServer(config)
	if err != nil {
...
// serverConfig creates a new dsn.Config struct based on an
// agent.Config struct and which can be used to initialize
// a DSN server
func (a *Agent) serverConfig() (*dsn.Config, error) {
