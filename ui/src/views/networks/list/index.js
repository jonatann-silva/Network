...
const NetworksView = () => {
  ...
  const handleNetworkDelete = (id) => {
    confirm({
      title: 'Tem certeza?',
      details: `Isso destruirá todos os recursos, incluindo interfaces e conexões, associados a essa rede.`,
      isDestructive: true,
      onConfirm: () => {
  ...
  return (
    <Container>
      <Box mb={3}>
        <Text textStyle="title">Redes</Text>
      ...
      {isError ? (
        ...
        <EmptyState message="Oops! Parece que você ainda não tem redes." />
      ) : (
        <InfiniteLoader
