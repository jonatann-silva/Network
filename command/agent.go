...
import (
	...
	agent "jonatann-silva/network/agent"
	cli "jonatann-silva/network/pkg/cli"
	log "jonatann-silva/network/pkg/log"
	simple "jonatann-silva/network/pkg/log/simple"
...
func (c *AgentCommand) Run(ctx context.Context, args []string) int {
	...
	c.UI.Output("==> Starting DSN agent...")
...
func (c *AgentCommand) Help() string {
	h := `
Usage: dsn agent [options]
	
  Starts the DSN agent and runs until an interrupt is received.
  The agent may be a client and/or server.
  
  The DSN agent's configuration primarily comes from the config
...
` + GlobalOptions() + `
  ...
  --config=<path>
    Path to a HCL file containing valid DSN configurations.
    Overrides the DRAGO_CONFIG_PATH environment variable if set.

  --data-dir=<path>
    The data directory where all state will be persisted. On DSN 
  ...
  --dev
    ...
    or testing DSN. No other configuration is required to start the
    agent in this mode.

  --log-level=<level>
    The logging level DSN should log at. Valid values are INFO, WARN,
    DEBUG, ERROR, FATAL. Overrides the DRAGO_LOG_LEVEL environment variable
    if set.

  --plugin-dir=<path>
    The plugin directory is used to discover DSN plugins. If not specified,
...
func (c *AgentCommand) printConfig() {
	...
	padding := 18
	c.UI.Output("==> DSN agent configuration:\n")
	for k := range info {
