...
func (c *NetworkInfoCommand) Run(ctx context.Context, args []string) int {
	...
	if len(args) != 1 {
		...
		c.UI.Error(`For additional help, try 'dsn network info --help'`)
		return 1
	}
...
func (c *NetworkInfoCommand) Help() string {
	h := `
  Usage: dsn network info <network> [options]

  Display detailed information about an existing DSN network.
